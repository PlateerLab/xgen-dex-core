...
macOS 에서는 시스템 입력기를 그대로 사용하고 **Caps Lock** 으로 한/영을 전환합니다.
`시스템 설정 → 키보드 → 텍스트 입력 → 편집`에서 `Caps Lock 키로 ABC 전환`을 켜
주세요. CLI 가 `Ctrl+Space`를 별도로 처리하지 않으므로 macOS 입력 소스 단축키와
상태가 이중으로 바뀌지 않습니다.

Linux와 Windows에서는 조합을 CLI 안에서 합니다 — 터미널의 IME 에 맡기지 않습니다.
잘 되는 터미널도 있지만 종류마다 다르고, SSH 나 tmux 를 거치면 아예 오지 않습니다.
표준 두벌식이고, 받침이 넘어가는 자리(`간` + `ㅏ` → `가나`)와 백스페이스로 되짚는
자리(`한` → `하` → `ㅎ`)가 실제 IME 와 같습니다. Caps Lock 이 켜져 있어도 같은
글자가 나옵니다.

Linux와 Windows의 CLI 자체 한/영 모드는 이 중 아무 것으로나 바꿉니다:

| 키 | 어디서 |
|---|---|
| `Ctrl+Space` | 어디서나 |
| `Shift+Space` | 어디서나 — 리눅스 입력기의 오랜 한/영 자리 |
| `Alt+Space` | 어디서나 — 한국어 자판의 한/영 키가 오른쪽 Alt 자리라 엄지로 닿습니다 |
| `Ctrl+L` | 터미널이나 tmux 가 `Ctrl+Space` 를 먼저 채갈 때 |
| **한/영 키**, **Caps Lock** | 아래 참고 |

지금 어느 쪽인지는 입력창 왼쪽(`한` / `EN`)과 아래 상태바(`[한]` / `[EN]`)에 늘 보입니다.
한국어 로케일이면 한글로 시작하고, 한 번 바꾸면 그 선택을 기억합니다.

**한/영 키와 Caps Lock 이 되는 곳** — 이 키들은 글자를 만들지 않아서, 보통은 터미널이
앱에 알려 주지 않습니다. [Kitty 키보드 프로토콜](https://sw.kovidgoyal.net/kitty/keyboard-protocol/)
을 아는 터미널(kitty · ghostty · WezTerm · foot · 최근 Alacritty · Konsole 등)에서만
그 키들이 도착하고, 그럴 때는 한/영 키(오른쪽 Alt 자리)와 Caps Lock 이 그대로 한/영
전환이 됩니다. 이름으로 아는 터미널이면 바로 켜고, 아니면 시작할 때 물어봐서 되는
경우에만 켜므로 모르는 터미널에서는 아무 일도 일어나지 않습니다(`DEX_NO_KITTY=1` 로
끌 수 있습니다).

macOS처럼 OS 입력기를 쓰는 경우, 입력기가 만들어 준 한글은 CLI 조합기를 거치지 않고
그대로 들어갑니다.
...
확장은 대화를 직접 하지 않고 CLI 엔진을 자식 프로세스로 띄워 씁니다. CLI 가 없으면
**확장이 처음 켜질 때 물어보고 같은 버전으로 깔아 줍니다** — 미리 설치해 두어도 됩니다.
