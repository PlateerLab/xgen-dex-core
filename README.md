...
조합은 CLI 안에서 합니다 — 터미널의 IME 에 맡기지 않습니다. 잘 되는 터미널도 있지만
종류마다 다르고, SSH 나 tmux 를 거치면 아예 오지 않습니다. 표준 두벌식이고, 받침이
넘어가는 자리(`간` + `ㅏ` → `가나`)와 백스페이스로 되짚는 자리(`한` → `하` → `ㅎ`)가
실제 IME 와 같습니다. Caps Lock 이 켜져 있어도 같은 글자가 나옵니다.

한/영은 이 중 아무 것으로나 바꿉니다:
...
OS 에 한국어 입력기가 있고 그것이 터미널에서 잘 동작한다면 그대로 쓰셔도 됩니다 —
입력기가 만들어 준 한글은 조합기를 거치지 않고 그대로 들어갑니다.
