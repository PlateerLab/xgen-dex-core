...
test('공유 확인 문구가 **공개되지 않는 것**을 말한다', () => {
  const v = code(VIEW)
  const share = v.slice(v.indexOf('onToggleShare'), v.indexOf('onDelete'))
  assert.match(share, /로그인 없이/, '무엇이 공개되는지 말해야 한다')
  assert.match(share, /API 는 공개 화면에서 동작하지/,
    '선언한 API 가 공개 화면에서 안 된다는 사실을 말하지 않으면, 사람은 되는 줄 알고 공유한다')
  assert.match(share, /권한/,
    '왜 안 되는지(보는 사람의 권한으로 나간다)를 말해야 다음에 무엇을 할지 안다')
})
