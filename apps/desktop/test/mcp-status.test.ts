// 채팅창 [로컬 컨트롤] 인디케이터의 상태별 문구를 검증한다.
...
test('전역 MCP가 꺼지면 비활성 상태를 표시한다', () => {
  ...
  assert.equal(status.label, '로컬 컨트롤 꺼짐');
});

test('소켓 연결과 카탈로그 ACK 대기를 구분한다', () => {
  assert.equal(mcpChatStatus({ ...base, connected: false }).label, '로컬 컨트롤 연결 중');
